...
async function main() {
  try {
    ...
    for (let user of userList) {
      ...
      // 运行前查积分（用于计算本次真实获得的积分）
      const beforeScore = await user.getSignInfo();
      // 签到
      ...
      if (user.ckStatus) {
        ...
        await $.wait(user.getRandomTime());
        //查询当前积分
        const afterScore = await user.getSignInfo();
        // 本次真实获得积分 = 运行后积分 - 运行前积分
        const gained = (beforeScore != null && afterScore != null) ? (Number(afterScore) - Number(beforeScore)) : 0;
        $.title = `本次运行共获得${gained}积分`;
        DoubleLog(`「${user.userName}」当前积分:${afterScore}分,累计签到:${count}天`);
      } else {
        //将ck过期消息存入消息数组
...
class UserInfo {
  ...
  async createArticle() {
    try {
      const opts = {
        ...
        type: "post",
        dataType: "json",
        headers: Object.assign(this.headers, getSign('app', {}, {postcontent: "开心的一天"})),
        body: {
          postcontent: "开心的一天"
        }
      }
      ...
      if (res?.code == '10000' && res?.message == '操作成功') {
        ...
        $.log(`⛔️ 创建动态: ${res?.message || JSON.stringify(res)}`);
      }
    } catch (e) {
      this.ckStatus = false;
      $.log(`⛔️ 创建动态失败! ${e}`);
  ...
  async getArticles() {
    try {
      const opts = {
        ...
        type: "get",
        headers: Object.assign(this.headers, getSign('app', {userId: this.userId})),
        dataType: "json",
        params: {
          userId: this.userId
        }
      ...
      if (res?.code == '10000' && res?.message == '操作成功') {
        ...
        $.log(`⛔️ 获取动态列表: ${res?.message || JSON.stringify(res)}`);
      }
    } catch (e) {
      this.ckStatus = false;
      $.log(`⛔️ 获取动态列表失败! ${e}`);
  ...
  async thumbsUp(postId) {
    try {
      const opts = {
        ...
        type: "post",
        headers: Object.assign(this.headers, getSign('app', {}, {postId, kindFlag:"0"})),
        dataType: "json",
        body: {
          postId,
          kindFlag:"0"
      ...
      if (res?.code == '10000') {
        ...
        $.log(`⛔️ 点赞动态: ${res?.message || JSON.stringify(res)}`)
      }
    } catch (e) {
      this.ckStatus = false;
      $.log(`⛔️ 点赞动态失败! ${e}`);
  ...
  async share(postId) {
    try {
      ...
      if (res?.code == '10000') {
        ...
        $.log(`⛔️ 分享动态: ${res?.message || JSON.stringify(res)}`)
      }
    } catch (e) {
      this.ckStatus = false;
      $.log(`⛔️ 分享失败! ${e}`);
  ...
  async deletePost(postId) {
    try {
      const params = { articleId: postId, postType: 1 }
      const opts = {
      ...
      if (res?.code == '10000') {
        ...
        $.log(`⛔️ 删除动态: ${res?.message || JSON.stringify(res)}`)
      }
    } catch (e) {
      this.ckStatus = false;
      $.log(`⛔️ 删除动态失败! ${e}`);
...
function getSign(type, params = {}, body = null) {
  ...
  // 统一签名算法：query + bodyJSON + appId=...&nonce=...&timestamp=... + appSecret
  const signature = `${query}${bodyStr}appId=${appConfig.appId}&nonce=${nonce}&timestamp=${timestamp}${appConfig.appSecret}`
  return {
...
async function Request(o) {
  ...
  try {
    ...
    const timeout = o.timeout ? ($.isSurge() ? o.timeout / 1e3 : o.timeout) : 3e4
    // 根据jsonType处理headers
